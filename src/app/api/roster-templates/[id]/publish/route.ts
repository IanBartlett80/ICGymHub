...
export async function POST(
  ...
  try {
    ...
    // Update all rosters linked to this template
    ...
    return NextResponse.json(
      {
        message: `Template rosters ${unpublish ? 'unpublished' : 'published'} successfully`,
        count: result.count,
      },
      { status: 200 }
    )
  } catch (error) {
    console.error('Failed to update template rosters', error)
    return NextResponse.json(
